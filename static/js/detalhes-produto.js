document.addEventListener('DOMContentLoaded', async function () {
    ...
    // Stats do produto (rating + vendas) abaixo do título
    const productStatsEl = document.getElementById('product-stats');
    if (productStatsEl) {
        const avs = produto.avaliacoes || [];
        const mediaP = avs.length > 0 ? (avs.reduce((s, a) => s + a.nota, 0) / avs.length) : 0;
        const vendidos = produto.total_vendidos || 0;
        const starSvg = `<svg width="13" height="13" fill="#f59e0b" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>`;
        const ratingHtml = mediaP > 0
            ? `<span style="display:flex;align-items:center;gap:3px;font-size:13px;color:#92400e;font-weight:600;">${starSvg} ${mediaP.toFixed(1)} <span style="font-weight:400;color:#64748b;">(${avs.length} avaliação${avs.length !== 1 ? 'ões' : ''})</span></span>`
            : `<span style="font-size:13px;color:#94a3b8;">Sem avaliações</span>`;
        const vendasHtml = vendidos > 0
            ? `<span style="font-size:13px;color:#64748b;">· ${vendidos} vendido${vendidos !== 1 ? 's' : ''}</span>`
            : `<span style="font-size:13px;color:#94a3b8;">· Nenhuma venda ainda</span>`;
        productStatsEl.innerHTML = ratingHtml + vendasHtml;
        productStatsEl.style.display = 'flex';
    }
